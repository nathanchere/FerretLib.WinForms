...
namespace FerretLib.WinForms.Controls
{
    public static class RageMessageBox
    {
        ...
        private partial class RageMessageBoxForm : Form
        {
            ...
            public void InitializeButtons(RageMessageBoxButtons buttons)
            {
                if (buttons == RageMessageBoxButtons.OK || buttons == RageMessageBoxButtons.OKCancel)
                    AddButton("OK", DialogResult.OK);

                if (buttons == RageMessageBoxButtons.AbortRetryIgnore)
                    AddButton("Abort", DialogResult.Abort);

                if (buttons == RageMessageBoxButtons.AbortRetryIgnore || buttons == RageMessageBoxButtons.RetryCancel)
                    AddButton("Retry", DialogResult.Retry);

                if (buttons == RageMessageBoxButtons.YesNo || buttons == RageMessageBoxButtons.YesNoCancel)
                {
                    AddButton("Yes", DialogResult.Yes);
                    AddButton("No", DialogResult.No);
                }

                if (buttons == RageMessageBoxButtons.RetryCancel || buttons == RageMessageBoxButtons.OKCancel || buttons == RageMessageBoxButtons.YesNoCancel)
                    AddButton("Cancel", DialogResult.Cancel);

                if (buttons == RageMessageBoxButtons.AbortRetryIgnore)
                    AddButton("Ignore", DialogResult.Ignore);

                if (buttons == RageMessageBoxButtons.FFFFFUUUUUUUU || buttons == RageMessageBoxButtons.Derp || buttons == RageMessageBoxButtons.LOL)
